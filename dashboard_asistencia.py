...
with c1:
    ...
    fig_pie = px.pie(df_filt, names='Estado', hole=0.4, color_discrete_sequence=px.colors.qualitative.Light24)
    st.plotly_chart(fig_pie, use_container_width=True)

with c2:
    st.subheader("Asistencia por Equipo")
    df_bar_team = df_filt.groupby(['Equipo', 'Estado']).size().reset_index(name='Cantidad')
    fig_bar_team = px.bar(df_bar_team, x='Equipo', y='Cantidad', color='Estado', barmode='group',color_discrete_sequence=px.colors.qualitative.Light24)
